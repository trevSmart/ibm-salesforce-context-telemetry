...
function showConfirmDialog({title, message, confirmText = 'Confirm', cancelText = 'Cancel', destructive = false}) {
	return new Promise((resolve) => {
		...
		const iconHtml = destructive ? `
			<div class="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full ${isDark ? 'bg-red-500/10' : 'bg-red-100'} sm:mx-0 sm:size-10">
							...
							${destructive ? `
								<div class="sm:flex sm:items-start">
									...
									<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
										...
										<div class="mt-2">
											<p class="text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}">${escapeHtml(message)}</p>
										</div>
									</div>
								</div>
								<div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
									<button type="button" data-action="confirm" class="inline-flex w-full justify-center rounded-md ${isDark ? 'bg-red-500 hover:bg-red-400' : 'bg-red-600 hover:bg-red-500'} px-3 py-2 text-sm font-semibold text-white shadow-xs sm:ml-3 sm:w-auto">${escapeHtml(confirmText)}</button>
