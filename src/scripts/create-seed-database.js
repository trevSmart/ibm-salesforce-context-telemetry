...
async function createSeedDatabase() {
	...
	try {
		...
		process.env.COPILOT_USERNAME = copilotUsername;
		process.env.COPILOT_PASSWORD = copilotPassword;
		process.env.COPILOT_ROLE = copilotRole;
		...
		// Create administrator user (if not already created by Copilot user)
