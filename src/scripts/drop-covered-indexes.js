...
/**
 * Script to drop covered/duplicate indexes from database (local or production)
 *
 ...
 *
 * Usage:
 *   node src/scripts/drop-covered-indexes.js [--env=local|prod] [--confirm]
 *
 ...
 *   node src/scripts/drop-covered-indexes.js
 *
 *   # Drop indexes from local database
 *   node src/scripts/drop-covered-indexes.js --confirm
 *
 *   # Drop indexes from production database
 *   DATABASE_URL_PROD="..." node src/scripts/drop-covered-indexes.js --env=prod --confirm
 *
 * IMPORTANT: Always backup before dropping indexes!
 */
...
async function checkIndexes(pool) {
	console.log('🔍 Checking which indexes exist...\n');
...
async function main() {
	...
	if (env === 'prod') {
		if (!process.env.DATABASE_URL_PROD) {
			...
			console.error('');
			console.error('Usage:');
			console.error('  DATABASE_URL_PROD="postgresql://user:pass@host/db" node src/scripts/drop-covered-indexes.js --env=prod [--confirm]');
			process.exit(1);
		}
	...
	try {
		...
		console.log(`⚠️  WARNING: This will drop indexes from ${envName} database!`);
