 ...
 *
 * Ús:
 *   node scripts/delete-user-events.js "Trevor Smart"
 *
 ...
 *
 * Exemple:
 *   DATABASE_URL="postgresql://user:pass@host/db" DATABASE_SSL=true node scripts/delete-user-events.js "Trevor Smart"
 */
...
if (!userName) {
	...
	console.error('Ús: node scripts/delete-user-events.js "Nom Usuari"');
	process.exit(1);
}
...
async function deleteUserEvents() {
	try {
		...
		if (!confirmFlag) {
			...
			console.log(`node scripts/delete-user-events.js "${userName}" --confirm`);
			await pool.end();
			return;
		}
